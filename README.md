...
- **Tamper detection.** Each batch is encrypted with AES-256-GCM, with a per-target monotonic sequence number and the target name bound into the GCM Additional Authenticated Data (AAD). The sequence number is stored in the blob header (adding 8 bytes per batch on disk) so the exporter can reconstruct the AAD at read-time. On clean shutdown, a per-target *seal batch* is written whose sequence number equals the count of data batches, giving the exporter a high-water-mark. [LogExporter](src/LogExporter.cpp) decrypts every blob, verifies per-target seqnums are contiguous with no gaps or duplicates, and checks every seal against the data-batch count. Any tag failure, gap, duplicate, or seal above the present count aborts the export and deletes any partial output.
  - **Detected:** batch deletion, duplication or replay, bit-flips within a batch, moves between target files, mid-stream truncation, and (up to the newest seal) tail truncation.
  - **Not detected:** restoration of an older full-directory snapshot (rollback), truncation to exactly an older seal boundary (equivalent to rollback), or removal of the newest seal itself (which downgrades detection to "best-effort without truncation evidence" and emits a warning on export). Defending against rollback requires an external anchor (e.g. TSA, transparency log) and is out of scope.
- **Log directories persist across runs.** At startup the manager scans the existing segments and continues each target's sequence numbers where the previous run stopped ([LogExporter::recoverTargetStates](src/LogExporter.cpp)); each clean shutdown adds a seal at its run boundary, and the exporter validates the whole multi-run chain. One caveat: if a run crashes mid-write, the torn trailing blob is ignored on recovery, but a subsequent run appends after it — export of that segment will then fail at the torn blob. Truncating the torn tail (or rotating to a fresh segment on crash detection) is future work.
